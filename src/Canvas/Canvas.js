...
function Canvas() {
    ...
    const childToParent = (childData) => {
        setShowMenu(false);
        console.log(balls, childData, showMenu)
        if (childData){
            balls[childData.id].color = childData.color;
        }
    }

    const [parentData, setParentData] = useState('');

    const parentToChild = (parentData) => {                          
  ...
  return (
    <div>
        { showMenu ? <ColorMenu childToParent={childToParent} parentToChild={parentData}/> : null }
        <canvas ref={canvasRef} id="my_canvas"/>
    </div>
  );
}
